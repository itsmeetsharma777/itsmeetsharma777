...
for _,r,c,x,y in sorted(cells):
    ...
    svg.append(f'<g transform="translate({x:.1f},{y:.1f})">'+('<g filter="url(#glow)">' if vt>.52 else '')+
               f'<polygon points="{poly(left)}" fill="{rgb(shade(base,.36))}" stroke="#182531" stroke-opacity="0.65" stroke-width="1.7" stroke-linejoin="round"/>'+ 
               f'<polygon points="{poly(right)}" fill="{rgb(shade(base,.61))}" stroke="#182531" stroke-opacity="0.65" stroke-width="1.7" stroke-linejoin="round"/>'+ 
               f'<polygon points="{poly(top)}" fill="{rgb(base)}" stroke="#182531" stroke-opacity="0.65" stroke-width="1.25" stroke-linejoin="round"/>'+ 
               ('</g>' if vt>.52 else '')+'</g>')
