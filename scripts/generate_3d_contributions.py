...
# Neutral cool-grey raised tiles, matching the supplied reference.
BASE_A,BASE_B="#607588","#71899a"; BASE_SIDE="#304657"; BASE_DARK="#1c2a36"
# Bright emerald/neon contribution palette, matching the supplied reference.
PALETTE=[(8,110,45),(8,153,54),(12,190,63),(20,218,73),(42,238,83),(78,250,101),(138,255,132)]
HW,HH=16.5,8.25; ORIGIN_X,ORIGIN_Y=150.0,245.0; MAX_H=172.0; BASE_DEPTH=7.0
...
# Individual chunky grey base blocks with strong, readable seams.
for _,r,c,x,y in sorted(ground):
    ...
    svg.append(f'<polygon points="{poly([top[3],top[2],bottom_left[1],bottom_left[0]])}" fill="{BASE_DARK}" stroke="#101a22" stroke-width="0.8"/>')
    svg.append(f'<polygon points="{poly([top[2],top[1],bottom_right[0],bottom_left[1]])}" fill="{BASE_SIDE}" stroke="#172633" stroke-width="0.8"/>')
    fill=BASE_A if (c+r)%2==0 else BASE_B
    svg.append(f'<polygon points="{poly(top)}" fill="{fill}" stroke="#142532" stroke-width="1.8"/>')
    inner=[(x,y-HH+1.8),(x+HW-2.0,y),(x,y+HH-1.8),(x-HW+2.0,y)]
    svg.append(f'<polygon points="{poly(inner)}" fill="none" stroke="#a9bac6" stroke-opacity="0.48" stroke-width="0.85"/>')
    ...
    for dc in range(-2,3):
        for dr in range(-2,3):
            ...
            if 0<=cc<53 and 0<=rr<7 and (dc or dr):
                neighbour=counts[cc][rr]
                if neighbour<=0: continue
                dist=abs(dc)+abs(dr); w=1.0/(1.0+dist*1.35); ws += math.log1p(neighbour)/math.log1p(max_count)*w; wt += w
    field=ws/wt if wt else 0.0
    visual_t=max(t,field*0.78)
    # Zero-activity cells stay grey. Only actual or nearby active cells become green.
    if visual_t<0.105: continue
    h=8.0+visual_t*MAX_H
    base=PALETTE[min(len(PALETTE)-1,int(visual_t*(len(PALETTE)-1)))]
    top=[(0,-HH-h),(HW,-h),(0,HH-h),(-HW,-h)]; left=[(-HW,0),(0,HH),(0,HH-h),(-HW,-h)]; right=[(0,HH),(HW,0),(HW,-h),(0,HH-h)]
    if visual_t>0.14: svg.append(f'<ellipse cx="{x:.1f}" cy="{y+3:.1f}" rx="13" ry="5" fill="#020806" opacity="0.38" filter="url(#softShadow)"/>')
    svg.append(f'<g transform="translate({x:.1f},{y:.1f})">')
    if visual_t>0.52: svg.append('<g filter="url(#cubeGlow)">')
    svg.append(f'<polygon points="{poly(left)}" fill="{rgb(shade(base,0.36))}"/>'); svg.append(f'<polygon points="{poly(right)}" fill="{rgb(shade(base,0.61))}"/>'); svg.append(f'<polygon points="{poly(top)}" fill="{rgb(base)}" stroke="{rgb(shade(base,1.12))}" stroke-width="0.45"/>')
    if visual_t>0.52: svg.append('</g>')
    svg.append('</g>')
